...
Need to see which teams run which forking — for map packing or hand-off
personnel? Pull the start list from Navisport, drop in the forking CSV from
the race organiser, and get a filterable, sortable view with CSV and
print/PDF export.

<details>
<summary>Details</summary>

Relay forking viewer for map packing and exchange personnel. Loads the
team/start list from [Navisport](#navisport) (supports relay, multistage, and
individual events via slug or UUID) and merges it with a forking CSV
...
| [`sprintticup-event_logo.html`](sprintticup-event_logo.html) | SprinttiCup event logo card generator (1080×1080 PNG): event details from Navisport, map background, embedded official logo, drag-to-position layout |
